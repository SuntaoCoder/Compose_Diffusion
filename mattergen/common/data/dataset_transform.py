...
def filter_sparse_properties(dataset: BaseDataset) -> BaseDataset:
    """
    Filter out structures with missing properties. 该函数用于过滤掉那些缺少某些属性的结构。
    Returns a new dataset with only structures that have all properties. 返回一个新的数据集，其中的每个结构都具有完整的属性。
    """
