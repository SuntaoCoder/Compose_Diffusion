...
class NumAtomsVarianceAdjustedWrappedVESDE(WrappedVESDE):
    ...
    用原子数调整方差的VESDE包裹。我们将标准偏差除以原子数的立方根。
    目标是减少单元大小对分数坐标方差的影响。
    
    是一个调整标准差的 VESDE 类，通过减少原子数量对分数坐标方差的影响，使模型对不同大小的结构具有更好的鲁棒性。
    """
    ...
    def marginal_prob(
        ...
        """
        1. 调用父类的 marginal_prob 方法，计算输入数据 x 在时间步 t 下的边际概率分布的均值和标准差。
        2. 根据当前批次数据的特性（通过 batch 提供），对标准差进行缩放调整。
        3. 返回调整后的均值和标准差。
        """
        ...
        std = std * maybe_expand(std_scale, batch_idx, like=std)    # 标准差缩放
        return mean, std
