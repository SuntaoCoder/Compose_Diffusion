...
@dataclass
class ComposeCrystalGenerator:
    checkpoint_info: list[MatterGenCheckpointInfo] | None = None
    ...
    _model_list: list[DiffusionLightningModule] | None = None
    _cfg_list: list[DictConfig] | None = None
    ...
    def num_conditions(self):
        return len(self.checkpoint_info)

    def prepare(self) -> None:
        """Loads the model from checkpoint and prepares for generation."""
        if self._model_list is not None:
            return
        model_list = [load_model_diffusion(self.checkpoint_info[i]).to(get_device()) for i in range(self.num_conditions())]

        self._model_list = model_list
        self._cfg_list = [self.checkpoint_info[i].config for i in range(self.num_conditions())]

    @property
    def model_list(self) -> DiffusionLightningModule:
        self.prepare()
        assert self._model_list is not None
        return self._model_list

    @property
    def cfg_list(self) -> DictConfig:
        self._cfg_list = [self.checkpoint_info[i].config for i in range(self.num_conditions())]
        assert self._cfg_list is not None
    ...
    def compose_generate(
        ...
        sampler = sampler_partial(pl_module=self.model_list)
