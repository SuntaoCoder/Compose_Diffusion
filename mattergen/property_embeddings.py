...
def replace_use_unconditional_embedding(
    ...
    try:
        ...
        # no existing data
        return batch.replace(**{_USE_UNCONDITIONAL_EMBEDDING: use_unconditional_embedding})    # 把这个字段 batch[_USE_UNCONDITIONAL_EMBEDDING] 设置为 Ture或者 False，形状为 [batch_size,1]
...
class SetEmbeddingType:
    def __init__(
        ...
        """
        self.p_unconditional = p_unconditional    # 在评分模型中使用无条件嵌入的概率
        self.dropout_fields_iid = dropout_fields_iid    # 见注释
...
class SetUnconditionalEmbeddingType:
    ...
    def __call__(self, x: ChemGraph) -> ChemGraph:
        # list of conditional fields present in the batch
        cond_fields = get_cond_field_names_in_batch(x=x)    # ['dft_mag_density']
...
class SetComposeUnconditionalEmbeddingType:

    def __call__(self, x: ChemGraph, cond_field_idx: int) -> ChemGraph:
        # list of conditional fields present in the batch
        cond_fields = get_cond_field_names_in_batch(x=x)    # ['dft_mag_density']
        ...
        return replace_use_unconditional_embedding(
            batch=x,
            use_unconditional_embedding={
                cond_fields[cond_field_idx]: torch.ones((len(x[cond_fields[cond_field_idx]]), 1), dtype=torch.bool, device=device)  # type: ignore
            },
        )
...
class SetConditionalEmbeddingType:
    ...
    def __call__(self, x: ChemGraph) -> ChemGraph:
        ...
        for cond_field in cond_fields:
            ...
            )    # use_unconditional_embedding {'dft_mag_density' : [[False],...,[False]]} 表示在该字段不使用无条件嵌入，使用有条件嵌入
...
class SetComposeConditionalEmbeddingType:
    ...
    def __call__(self, x: ChemGraph, cond_field_idx: int) -> ChemGraph:
        ...
        use_unconditional_embedding[cond_fields[cond_field_idx]] = torch.zeros(
            (len(x[cond_fields[cond_field_idx]]), 1), dtype=torch.bool, device=device
        )
