...
class ComposePredictorCorrector(Generic[Diffusable]):
    """Generates samples using predictor-corrector sampling."""

    def __init__(
        self,
        *,
        diffusion_module: list[DiffusionModule] | None = None,
        ...
        self.num_conditions = len(diffusion_module)
        self._diffusion_module = [diffusion_module[i].diffusion_module for i in range(self.num_conditions)]
    ...
    @property
    def diffusion_module(self) -> list[DiffusionModule]:
        return self._diffusion_module
    
    @property
    def _multi_corruption(self) -> MultiCorruption:
        return self._diffusion_module[0].corruption
    
    def _score_fn(self, x: Diffusable, t: torch.Tensor, idx: int) -> Diffusable:
        return self._diffusion_module[idx].score_fn(x, t)

    @classmethod
    def from_pl_module(cls, pl_module: list[DiffusionLightningModule] | None, **kwargs) -> ComposePredictorCorrector:
        return cls(diffusion_module=pl_module, device=pl_module[0].device, **kwargs)
    ...
    def _sample_maybe_record(
        ...
        """
        for module in self._diffusion_module:
