...
from mattergen.diffusion.sampling.pc_sampler_compose import ComposePredictorCorrector
...
class ComposeGuidedPredictorCorrector(ComposePredictorCorrector):
    ...
    def _score_fn(
        ...
        unconditional_score = [super(ComposeGuidedPredictorCorrector, self)._score_fn(x=self._remove_conditioning_fn(x, i), t=t, idx=i) for i in range(self.num_conditions)]
        conditional_score = [super(ComposeGuidedPredictorCorrector, self)._score_fn(x=self._keep_conditioning_fn(x, i), t=t, idx=i) for i in range(self.num_conditions)]

        def combine_scores(conditional_score, unconditional_score, guidance_factor, k):

            len_conditional_score = len(conditional_score)
            len_unconditional_score = len(unconditional_score)
            assert len_conditional_score == len_unconditional_score

            total = torch.zeros_like(unconditional_score[0][k])
            for uncond in unconditional_score:
                total += uncond[k]
            mean_part = total

            sum_part = torch.zeros_like(unconditional_score[0][k])
            for cond, uncond in zip(conditional_score, unconditional_score):
                sum_part += guidance_factor * (cond[k] - uncond[k])
            
            combine_score = mean_part + sum_part
            
            return combine_score

        return unconditional_score[0].replace(
            **{
                k: combine_scores(conditional_score, unconditional_score, self._guidance_scale, k)
                for k in self._multi_corruption.corrupted_fields
            }
        )
