...
def maybe_instantiate(instance_or_config: T | Mapping, expected_type=None, **kwargs) -> T:
    ...
    这段代码定义了一个名为 maybe_instantiate 的函数，它尝试根据传入的参数实例或配置字典来实例化一个对象。
    如果传入的参数是一个包含 _target_ 字段的映射（通常是一个字典），则使用这个字段指定的类来实例化对象。
    否则，如果传入的参数已经是一个实例，则直接返回这个实例。函数还检查实例化的对象是否符合预期的类型，如果不符合，则抛出一个断言错误。
    """
